...
class Auth_masyarakat extends CI_Controller
{
    ...
    public function index()
    {
        ...
        $data['page_masuk'] = 'Masuk';

        if ($this->form_validation->run() == false) {
            $this->load->view('masyarakat/template/header');
            $this->load->view('masyarakat/login', $data);
        ...
        if ($user) {
            if ($user['is_active'] == 1) {
                if (password_verify($password, $user['password'])) {
                    ...
                    $this->session->set_flashdata('message-error', '<div class="text-white text-center capitalize">Password salah</div>');
                    redirect('auth_masyarakat');
                }
            } else {
                $this->session->set_flashdata('message-warning', '<div class="text-white text-center capitalize">Akun anda belum diaktivasi</div>');
                redirect('auth_masyarakat');
            }
        } else {
            $this->session->set_flashdata('message-error', '<div class="text-white text-center capitalize">Username belum terdaftar</div>');
            redirect('auth_masyarakat');
        }
    }
    ...
    public function registration()
    {
        ...
        $data['page_register'] = 'Daftar';

        if ($this->form_validation->run() == false) {
            $this->load->view('masyarakat/template/header');
            $this->load->view('masyarakat/register', $data);
            $this->load->view('masyarakat/template/footer');
        } else {
            $data = [
            ...
            $this->session->set_flashdata('message-success', '<div class="text-white text-center capitalize">Berhasil register. Silahkan aktivasi akun <br> di email anda</div>');
            redirect('auth_masyarakat');
        }
    }
                                        ...
                                        <table width="95%" border="0" align="center" cellpadding="0" cellspacing="0" style="max-width:670px; background:#fff; border-radius:3px; text-align:center;-webkit-box-shadow:0 6px 18px 0 rgba(0,0,0,.06);-moz-box-shadow:0 6px 18px 0 rgba(0,0,0,.06);box-shadow:0 6px 18px 0 rgba(0,0,0,.06);">
                                            ...
                                            <tr>
                                                <td style="padding:0 60px;">
                                                    ...
                                                    <a href="' . base_url() . 'auth_masyarakat/resetpassword?email=' . $this->input->post('forgotPassword') . '&token=' . urlencode($token) . '" style="background:rgba(79, 70, 229, 1); text-decoration:none !important; font-weight:500; margin-top:35px; color:#fff; text-transform:uppercase; font-size:14px; padding:10px 24px; display:inline-block; border-radius:5px; cursor:pointer">
        ...
        if ($user) {
            ...
            if ($user_token) {
                if (time() - $user_token['date_created'] < (60 * 60 * 24)) {
                    ...
                    $this->session->set_flashdata('message-success', '<div class="text-white text-center capitalize">Aktivasi akun berhasil</div>');
                    redirect('auth_masyarakat');
                } else {
                    $this->db->delete('masyarakat', ['email' => $email]);
                    $this->db->delete('user_token', ['email' => $email]);

                    $this->session->set_flashdata('message-error', '<div class="text-white text-center capitalize">Aktivasi akun gagal, Token anda expired</div>');
                    redirect('auth_masyarakat');
                }
            } else {
                $this->session->set_flashdata('message-error', '<div class="text-white text-center capitalize">Aktivasi akun gagal, Token anda salah</div>');
                redirect('auth_masyarakat');
            }
        } else {
            $this->session->set_flashdata('message-error', '<div class="text-white text-center capitalize">Aktivasi akun gagal, Email anda salah</div>');
            redirect('auth_masyarakat');
        }
    }
    ...
    public function forgotPassword()
    {
        ...
        if ($user) {
            ...
            $this->session->set_flashdata('message-success-2', '<div class="text-white text-center capitalize">Silahkan cek akun email anda untuk reset password</div>');
            redirect('auth_masyarakat');
        } else {
            $this->session->set_flashdata('message-warning-2', '<div class="text-white text-center capitalize">Email belum terdaftar</div>');
            redirect('auth_masyarakat');
        }
    }
    ...
    public function resetPassword()
    {
        ...
        if ($user) {
            ...
            if ($user_token) {
                ...
                $this->session->set_flashdata('message-error', '<div class="text-white text-center capitalize">setel ulang password gagal,<br> token anda salah</div>');
                redirect('auth_masyarakat');
            }
        } else {
            $this->session->set_flashdata('message-error', '<div class="text-white text-center capitalize">setel ulang password gagal,<br> email anda salah</div>');
            redirect('auth_masyarakat');
        }
    }
    ...
    public function changePassword()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $this->session->set_flashdata('message-success', '<div class="text-white text-center capitalize">password berhasil dirubah. silahkan login</div>');
            redirect('auth_masyarakat');
        }
    }
